...
import type { GroupSize } from "../types/trip";
...
export default function GroupDuration({ dest, onNext, onBack }: Props) {
        ...
        <Wrap>
            <div style={{ maxWidth: 520, margin: "0 auto", padding: "32px 24px", animation: "fadeUp 0.6s ease both" }}>
                ...
                <div style={{ marginBottom: 32 }}>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                        {GROUP_OPTIONS.map((g) => {
                            ...
                            return (
                                <button key={g.id} onClick={() => setGroup(g.id)} style={{ background: selected ? "rgba(192,132,252,0.12)" : "rgba(255,255,255,0.03)", border: selected ? "1.5px solid rgba(192,132,252,0.5)" : "1.5px solid rgba(255,255,255,0.08)", borderRadius: 16, padding: "18px 14px", cursor: "pointer", textAlign: "left", transition: "all 0.2s ease", position: "relative", overflow: "hidden" }}>
                                    {selected && <div style={{ position: "absolute", top: 10, right: 10, width: 18, height: 18, borderRadius: "50%", background: "linear-gradient(135deg,#7c3aed,#c084fc)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, color: "#fff" }}>✓</div>}
                ...
                <div style={{ marginBottom: 32 }}>
                    ...
                    <div style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 20, padding: "28px", textAlign: "center" }}>
                        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 32, marginBottom: 22 }}>
                            <button onClick={() => setDays((d) => Math.max(1, d - 1))} style={{ width: 52, height: 52, borderRadius: "50%", border: "1.5px solid rgba(255,255,255,0.15)", background: "rgba(255,255,255,0.05)", color: "#fff", fontSize: 22, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'DM Sans',sans-serif" }}>−</button>
                            <div>
                            ...
                            <button onClick={() => setDays((d) => Math.min(14, d + 1))} style={{ width: 52, height: 52, borderRadius: "50%", border: "1.5px solid rgba(255,255,255,0.15)", background: "rgba(255,255,255,0.05)", color: "#fff", fontSize: 22, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'DM Sans',sans-serif" }}>+</button>
                        </div>
                        <div style={{ display: "flex", gap: 8, justifyContent: "center", flexWrap: "wrap" }}>
                            {QUICK_DAYS.map((d) => <button key={d} onClick={() => setDays(d)} style={{ padding: "6px 14px", borderRadius: 20, border: `1.5px solid ${days === d ? "#c084fc" : "rgba(255,255,255,0.1)"}`, background: days === d ? "rgba(192,132,252,0.15)" : "transparent", color: days === d ? "#c084fc" : "rgba(255,255,255,0.45)", fontSize: 13, cursor: "pointer", fontFamily: "'DM Sans',sans-serif", transition: "all 0.2s ease" }}>{d}d</button>)}
                        </div>
                    </div>
                </div>
                ...
                <div style={{ background: "rgba(192,132,252,0.05)", border: "1px solid rgba(192,132,252,0.15)", borderRadius: 14, padding: "14px 18px", marginBottom: 24, display: "flex", alignItems: "center", gap: 12 }}>
                    ...
                    <div>
                        <div style={{ color: "#fff", fontSize: 13, fontWeight: 500 }}>{GROUP_OPTIONS.find((g) => g.id === group)!.emoji} {GROUP_OPTIONS.find((g) => g.id === group)!.label} · {days} {days === 1 ? "day" : "days"} in {dest.main}</div>
                        <div style={{ color: "rgba(255,255,255,0.38)", fontSize: 12, marginTop: 2 }}>AI will personalise preferences for you next</div>
                    </div>
                </div>

                <button className="btn-primary" onClick={() => onNext(group, days)}>Choose Preferences →</button>
            </div>
        </Wrap>
    );
}
