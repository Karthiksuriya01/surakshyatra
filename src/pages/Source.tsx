...
import type { SourcePlace } from "../types/trip";
...
export default function Source({ onNext }: Props) {
    ...
    const reverseGeocode = (lat: number, lng: number) => {
        ...
        gc.geocode({ location: { lat, lng } }, (results, status) => {
            if (status !== google.maps.GeocoderStatus.OK || !results?.length) { setDetectState("denied"); return; }
            const best = results.find((r) => r.types.some((t) => ["locality", "sublocality"].includes(t))) ?? results[0];
            const { main, secondary } = extractPlace(best);
            setDetected({ placeId: best.place_id, main, secondary, emoji: "📍" });
                ...
                <div style={{ width: "100%", maxWidth: 520, background: "rgba(255,255,255,0.04)", backdropFilter: "blur(28px)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 24, padding: "28px", boxShadow: "0 32px 80px rgba(0,0,0,0.45)", animation: "fadeUp 0.7s ease 0.1s both" }}>
                    ...
                    {detectState === "detecting" && (
                        <div style={{ animation: "fadeUp 0.4s ease both" }}>
                            ...
                            <button onClick={() => setDetectState("manual")} style={{ marginTop: 14, width: "100%", padding: "12px", background: "transparent", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 14, color: "rgba(255,255,255,0.5)", fontSize: 13, cursor: "pointer", fontFamily: "'DM Sans',sans-serif", transition: "all 0.15s" }}>✏️ Type manually instead</button>
                        </div>
                    )}
                    ...
                    {detectState === "detected" && detected && (
                        <div style={{ animation: "fadeUp 0.4s ease both" }}>
                            ...
                            <button className="btn-primary" onClick={() => onNext(detected)} style={{ marginBottom: 10 }}>Use My Location → {detected.main}</button>
                            <button onClick={() => { setDetectState("manual"); setQuery(""); setSelected(null); setResults([]); }} style={{ width: "100%", padding: "12px", background: "transparent", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 14, color: "rgba(255,255,255,0.5)", fontSize: 13, cursor: "pointer", fontFamily: "'DM Sans',sans-serif", transition: "all 0.15s" }}>✏️ Search a different starting point</button>
                        </div>
                    )}
                    ...
                    {detectState === "denied" && (
                        <div style={{ animation: "fadeUp 0.4s ease both" }}>
                            ...
                            <button onClick={requestLocation} style={{ width: "100%", padding: "11px", background: "rgba(192,132,252,0.08)", border: "1px solid rgba(192,132,252,0.2)", borderRadius: 14, color: "#c084fc", fontSize: 13, cursor: "pointer", fontFamily: "'DM Sans',sans-serif", marginBottom: 10, transition: "all 0.15s" }}>🔄 Try again</button>
                            <button className="btn-primary" style={{ background: "rgba(255,255,255,0.07)", boxShadow: "none" }} onClick={() => setDetectState("manual")}>✏️ Search manually</button>
                        </div>
                    )}

                    {detectState === "manual" && (
                        <div style={{ animation: "fadeUp 0.3s ease both" }}>
                            <button onClick={requestLocation} style={{ display: "inline-flex", alignItems: "center", gap: 7, background: "rgba(16,185,129,0.08)", border: "1px solid rgba(16,185,129,0.2)", borderRadius: 30, padding: "6px 14px", fontSize: 12, color: "#10b981", cursor: "pointer", fontFamily: "'DM Sans',sans-serif", marginBottom: 16, transition: "all 0.15s" }}>
                            ...
                            <div style={{ position: "relative" }}>
                                <div style={{ display: "flex", alignItems: "center", gap: 11, background: focused ? "rgba(192,132,252,0.06)" : "rgba(255,255,255,0.04)", border: `1.5px solid ${focused ? "rgba(192,132,252,0.55)" : "rgba(255,255,255,0.1)"}`, borderRadius: showDrop ? "15px 15px 0 0" : "15px", padding: "13px 16px", transition: "all 0.2s", boxShadow: focused ? "0 0 0 4px rgba(192,132,252,0.07)" : "none" }}>
                                    {searchLoading ? <div style={{ width: 17, height: 17, borderRadius: "50%", border: "2px solid rgba(192,132,252,0.25)", borderTopColor: "#c084fc", animation: "spin 0.8s linear infinite", flexShrink: 0 }} /> : <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke={focused ? "#c084fc" : "rgba(255,255,255,0.3)"} strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0, transition: "stroke 0.2s" }}><circle cx="11" cy="11" r="8" /><path d="m21 21-4.35-4.35" /></svg>}
                                    <input className="sy-input" value={query} onChange={(e) => { const v = e.target.value; setQuery(v); if (selected && v !== selected.main) setSelected(null); }} onFocus={() => setFocused(true)} onBlur={() => setTimeout(() => setFocused(false), 300)} onKeyDown={onKey} placeholder="Search your city or address…" autoComplete="off" spellCheck={false} autoFocus />
                            ...
                            <div style={{ marginTop: 16 }}>
                                <button className="btn-primary" disabled={!selected} onClick={() => selected && onNext(selected)}>{selected ? `Continue from ${selected.main} →` : "Select Your Starting Point"}</button>
                            </div>
                        </div>
                    )}
                </div>
