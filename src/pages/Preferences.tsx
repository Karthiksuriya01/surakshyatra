...
import { fetchPrefSuggestions } from "../services/ai";
...
export default function Preferences({ dest, groupSize, days, onNext, onBack }: Props) {
    ...
    useEffect(() => {
        ...
        fetchPrefSuggestions(dest.main, groupSize)
            .then((res) => {
                const ordered = [...res.all.map((id) => ALL_PREFS.find((p) => p.id === id)!).filter(Boolean), ...ALL_PREFS.filter((p) => !res.all.includes(p.id))];
                setOrderedPrefs(ordered);
                setAiSuggested(res.suggested);
                setSel(res.suggested.slice(0, 3));
            })
            .catch(() => { setAiError(true); setSel(["popular"]); })
        ...
        <Wrap>
            <div style={{ maxWidth: 520, margin: "0 auto", padding: "32px 24px", animation: "fadeUp 0.6s ease both" }}>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 10, marginBottom: 24, minHeight: 50 }}>
                    {loading
                        ? Array.from({ length: 6 }).map((_, i) => <div key={i} style={{ height: 38, width: 100 + (i % 3) * 20, borderRadius: 20, background: "rgba(255,255,255,0.05)", animation: `pulse 1.4s ease ${i * 0.15}s infinite` }} />)
                        : orderedPrefs.map((p) => {
                            const isSelected = sel.includes(p.id);
                            const isAI = aiSuggested.includes(p.id);
                            return (
                                <button key={p.id} className={`pref-chip ${isSelected ? "selected" : ""}`} onClick={() => toggle(p.id)} style={{ position: "relative" }}>
                                    {p.emoji} {p.label}
                                    {isAI && !isSelected && <span style={{ marginLeft: 4, fontSize: 10, color: "#c084fc", opacity: 0.7 }}>✦</span>}
                ...
                <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 16, padding: "16px", marginBottom: 28 }}>
                    ...
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                        {sel.length === 0
                            ...
                            : sel.map((id) => { const p = ALL_PREFS.find((x) => x.id === id); if (!p) return null; return <span key={id} style={{ background: "rgba(192,132,252,0.12)", border: "1px solid rgba(192,132,252,0.25)", borderRadius: 20, padding: "3px 10px", fontSize: 12, color: "#c084fc" }}>{p.emoji} {p.label}</span>; })}
                    </div>
                </div>
