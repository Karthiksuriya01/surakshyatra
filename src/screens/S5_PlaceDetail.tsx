...
export default function S5_PlaceDetail({ place, onBack }: Props) {
            ...
            <div style={{ maxWidth: 560, margin: "0 auto", animation: "scaleIn 0.4s cubic-bezier(0.16,1,0.3,1) both" }}>
                ...
                <div style={{ padding: "0 24px 40px" }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 20 }}>
                        {[1, 2, 3, 4, 5].map((i) => (
                            <span key={i} style={{ fontSize: 16, color: i <= Math.round(place.rating) ? "#f59e0b" : "rgba(255,255,255,0.15)" }}>★</span>
                        ))}
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 16 }}>
                        {[
                            { icon: "🕐", label: "Best Time", val: place.time },
                            { icon: "⏱️", label: "Duration", val: place.duration },
                            { icon: "📏", label: "Distance", val: place.distance },
                            { icon: "🟢", label: "Status", val: place.open },
                        ].map((info, i) => (
